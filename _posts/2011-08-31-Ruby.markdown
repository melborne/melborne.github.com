...
gem install ambでインストールして次のように使うよ{% fn_ref 3 %}
まずはピタゴラスから
{% highlight ruby %}
require 'amb'
def pythag(n)
...
ans = [*1..5].permutation(5)tap{|t| p t}.select do |baker, cooper, fletcher, miller, smith|<br><br>tapの前の.が抜け落ちている。<br><br>ans = [*1..5].permutation(5).tap{|t| p t}.select do |baker, cooper, fletcher, miller, smith|>kumonopanyaさん<br>ありがとう！訂正しました
{% footnotes %}
